...
def extract_trait_matrix(accessions: list[dict]) -> tuple[np.ndarray, list[str]]:
    ...
    for acc in accessions:
        ...
        raw.append(vec)
        labels.append(f"{acc['genus']} {acc['species']} — {acc['cultivar']} ({acc['origin_country']})")
...
class HilbertMatcher:
    ...
    def kernel_matrix_info(self) -> dict:
        ...
        return {
            ...
            "kernel_det": round(float(np.linalg.det(self.K)) if self.K.shape[0] <= 12 else -1, 8),
...
def print_results(results: list[dict], query_name: str):
    ...
    for r in results:
        ...
        if len(sorte) > 45:
            sorte = sorte[:42] + "..."
        print(f"  {r['rank']:<3} {r['match_score']:>5.1f}%  {sorte:<45} {r['origin']:<12}")
