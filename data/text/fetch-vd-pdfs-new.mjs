// VD-PDF download for the M2a crops — variant of fetch-vd-pdfs.mjs reading the
// NEW coverage file (data/text/cpvo-coverage-new.json, committed) instead of
// /tmp/cpvo-coverage.json. Pipeline identical (all anonymous GETs):
//   publicSearchDocuments/{appNo} → VD doc (csDocsKey) → downloadPublic/{csDocsKey}
//   → presigned S3 → PDF
// Output: /tmp/vd-pdfs-new + manifest /tmp/vd-manifest-new.json (PDFs stay out
// of the repo; the manifest is the delta-embed pipeline's input).
import fs from 'node:fs'

const coverage = JSON.parse(fs.readFileSync(new URL('./cpvo-coverage-new.json', import.meta.url), 'utf8'))
  .filter((r) => r.status === 'FOUND')
const API = 'https://online.plantvarieties.eu/api'
const HDRS = { environment: 'PRO', 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64)' }
const OUT_DIR = '/tmp/vd-pdfs-new'
...
for (const v of coverage) {
  ...
  const entry = { crop: v.crop, name: v.name, applicationNumber: v.applicationNumber, file: null }
  try {
...
fs.writeFileSync('/tmp/vd-manifest-new.json', JSON.stringify(manifest, null, 1))
