...
// ── Loop-It 36: BSA-Echtdaten-Deckel gepinnt (224 offizielle BSL-2026-Sorten) ──
{
  ...
  check(`BSA-Real: 362 Sorten über 8 Fruchtarten im Union-Space (aktuell ${union.rows.length})`,
    union.rows.length === 362)
  const byCrop: Record<string, number[]> = {}
  union.crops.forEach((crop, index) => { (byCrop[crop] ??= []).push(index) })
  const ceilings: Record<string, number> = { Weizen: 1, Gerste: 1, Roggen: 1, Dinkel: 1, Hafer: 0.94, Raps: 0.82, Ackerbohne: 0, Mais: 0.83 }
